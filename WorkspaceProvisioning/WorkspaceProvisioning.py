...
if (licenseResponse.status_code != 200 or len(licenseResponse.json()['items']) == 0):
    ...
    for license in licenseResponse.json()['items']:
        ...
        elif (license['name'] == "Webex Calling - Workspaces"):
            workspaceLicenseId = license['id']
        else:
            continue
...
while (loopCount < totalUsers and callingLicenseId != ''):
    ...
    if (users['loopCount']['License'] == 'professional'):
        licenseId = callingLicenseId
    else:
        licenseId = workspaceLicenseId

    workspaceBody = json.dumps({
        "displayName": users['loopCount']['Name'],
    ...
    workspaceResponse = requests.patch(workspaceUrl, headers=workspaceeHeaders, data=workspaceBody)
